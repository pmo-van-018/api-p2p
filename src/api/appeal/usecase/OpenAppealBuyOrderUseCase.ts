...
@Service()
export class OpenAppealBuyOrderUseCase {
  constructor(
  ...
  public async openAppealBuyOrder(currentUser: User, orderRefId: string): Promise<string> {
    return await RedlockUtil.lock(orderRefId, async () => {
      const order = await this.orderService.getBuyOrderByRefIdWithLock(orderRefId);
      ...
      const nextStep = await this.orderService.updateBuyOrderToAppealStep(order.id, order.step);
