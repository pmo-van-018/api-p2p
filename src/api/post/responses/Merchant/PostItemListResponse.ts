...
export class PostItemListResponse {
  ...
  constructor(post: Post) {
    ...
    if (this.isShowTotalFee(post.status)) {
      this.totalFee = Number(post.totalFee);
      this.totalPenaltyFee = Number(post.totalPenaltyFee);
    }
    this.fee = post.merchant?.masterDataLevel?.fee;
    }

  private isShowTotalFee(status: PostStatus): boolean {
    return status === PostStatus.CLOSE;
  }
}
