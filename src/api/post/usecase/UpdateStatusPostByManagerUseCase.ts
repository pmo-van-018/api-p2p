...
import {OperationStatus, PostStatus, TradeType} from '@api/common/models/P2PEnum';
...
@Service()
export class UpdateStatusPostByManagerUseCase {
  constructor(
  ...
  public async updateStatusPost(
    ...
    const post = await this.postService.getFullInfoPostByManagerId({ refId, merchantManagerId: currentUser.id });
    if (!post || post.merchant.status !== OperationStatus.ACTIVE) {
      return PostError.POST_ID_IS_INVALID;
    }
    if (post.status === PostStatus.CLOSE) {
      return PostError.CANNOT_UPDATE_CLOSED_POST;
    }
    if (currentUser.id !== post.merchant.merchantManagerId) {
    ...
    this.eventDispatcher.dispatch(
      showAd === PostStatus.ONLINE
        ? events.actions.operator.onlinePostByManager
        : events.actions.operator.offlinePostByManager,
      {
        staff: post.merchant,
        refId: post.refId,
      }
