...
import { socketLoader } from '../../src/loaders/socketLoader';
...
export const bootstrapApp = async (): Promise<BootstrapSettings> => {
  const framework = await bootstrapMicroframework({
    loaders: [winstonLoader, iocLoader, eventDispatchLoader, typeormLoader, expressLoader, homeLoader, socketLoader],
  });
  return {
