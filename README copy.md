...
```
yarn start:dev:api --help 
```
Display help for dev serve command.

```
yarn start:dev:worker --help 
```
Display help for dev worker command.

```
yarn start:prod:api --help 
# or
node dist/index.js --help
```
Display help for production serve command.

```
yarn start:prod:worker --help 
# or
node dist/index-worker.js --help
```
Display help for production worker command.


```
yarn run setup
```
Clear database, chạy lại migrate: (lệnh này chỉ dùng 1 lần đầu tiên)

```
yarn start
```
Re-build + Run on Production

```
yarn start db.migrate
```
Chạy các file migrate - update database

```
yarn start db.seed
```
Insert 1 số data ban đầu vô database

```
yarn generate xxxx-name
```
Tạo tự động file migrate tên xxx-name
```
yarn debug
```
DEV debug trên vscode local

---

## Đối với env STG, PRODUCTION: thì chạy 2 lệnh
```
yarn start db.migrate
yarn start
```

NOTE: Bởi vì trong code xài tslint, eslint, nên code mà bị thừa thiếu, dư biến...
Khi build/commit nó sẽ báo lỗi, nên DEV phải sửa hết các lỗi đó mới chạy được.
...
Or you can easy setup docker image for the db (MySQL, Redis), migration, seeding the DB, and installing the dependencies:
